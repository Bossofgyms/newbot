    ...
    while True:
        try:
            ...
            target_time = now.replace(
                hour=DAILY_TIME_HOUR, minute=DAILY_TIME_MINUTE, second=0, microsecond=0
            )
            ...
            print(
                f"Следующая отправка гороскопов через {hours} часов {minutes} минут ({target_time.strftime('%d.%m.%Y %H:%M')})"
            )
